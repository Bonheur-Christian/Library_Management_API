...
const NovelModel = {
    ...
    insertBookInStock: async (title, author, isbn, price, published_year, quantity) => {
        const query = "INSERT INTO novels(bookName, book_author , bookISBN, price , published_year, quantity) VALUE(?,?,?,?,?,?)";
        try {
            const [similarBook] = await connection.execute("SELECT * FROM novels WHERE bookISBN = ?", [isbn]);
    ...
    getBookFromStock: async (id) => {
        const query = "SELECT * FROM novels WHERE bookID  =?";
        try {
    ...
    updateBookInStock: async (title, author, isbn, price, published_year, quantity, id) => {
        const updateQuery = "UPDATE novels SET bookName =?, book_author =?, bookISBN =?, price =?, published_year =? , quantity =? WHERE bookID =?";
        try {
    ...
    deleteBookFromStock: async (id) => {
        const deleteQuery = "DELETE FROM novels WHERE bookID = ?";
